...
import { statGrid, stat, macroChips } from '../ui/components.js';
...
export function renderDzis(root, ctx) {
  ...
  const series = r.training ? r.training.reduce((a, e) => a + e.seriesToday, 0) : 0;
  ...
  add(root, 
    ...
    h('div', { class: 'row quicklinks' },
      h('a', { class: 'btn ql', href: `#/dieta?f=${r.phase ?? 0}&w=${r.dietVariant}` }, 'Jadłospis dnia'),
      h('a', { class: 'btn ql', href: `#/suplementy?d=${date}` }, 'Suplementacja dnia')),
    h('div', { class: 'row daynav' }, nav(-1, 'Poprzedni dzień'), date !== ctx.today && h('a', { class: 'btn', href: '#/dzis' }, 'Dziś'), nav(1, 'Następny dzień')),
    now && (now.current || now.next) ? h('div', { class: 'nowcard' },
      ...
      now.current && h('a', { class: 'btn', href: `#${now.current.id}` }, 'Pokaż w planie')) : null,
    statGrid(
      stat('Dieta', `${r.kcal} kcal`, `${r.dietVariant === 'T' ? 'dzień treningowy' : 'dzień nietreningowy'} · B ${t.p} / W ${t.c} / T ${t.f} g`),
      stat('Trening', r.sessionLabel, series ? `${series} serii w Fazie ${r.phase ?? 0}` : 'bez serii siłowych'),
      stat('CFA', r.cfa.inPlan ? `${r.cfa.blocks.length} bloków` : 'poza planem', r.cfa.inPlan ? `${cfaMin} min${r.cfa.recall ? ' + 53 min recall' : ' · bez recall'}` : null),
      stat('Sauna', r.sauna ? `${r.sauna} ${r.sauna === 1 ? 'runda' : 'rundy'}` : 'brak', r.sauna ? '19:45 / protokół' : 'dzień bez sauny')),
