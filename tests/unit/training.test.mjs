...
const T = {
  [`2026-09-21|${bench}|1`]: { date: '2026-09-21', ex: bench, set: 1, done: true, kg: 45, reps: 10, rir: 2 },
  [`2026-09-21|${bench}|2`]: { date: '2026-09-21', ex: bench, set: 2, done: true, kg: 45, reps: 8, rir: 2 },
  [`2026-09-28|${bench}|1`]: { date: '2026-09-28', ex: bench, set: 1, done: true, kg: 50, reps: 8, rir: 2 },
  [`2026-09-22|${rdl}|1`]: { date: '2026-09-22', ex: rdl, set: 1, done: false, kg: null, reps: null, rir: null },
};
...
test('sesje z czasem trwania i agregaty tygodniowe', () => {
  const ss = sessions(T, { '2026-09-21': { date: '2026-09-21', minutes: 75 } });
  assert.deepEqual(ss.map(x => [x.date, x.sets, x.minutes]), [['2026-09-21', 2, 75], ['2026-09-28', 1, null]]);
  const w = weekly(T, { '2026-09-21': { minutes: 75 } }, 2, '2026-09-30');
  assert.deepEqual(w.map(x => [x.week, x.sessions, x.minutes]), [['2026-09-21', 1, 75], ['2026-09-28', 1, 0]]);
});

test('serie na grupę mięśni: główny 1, pomocniczy 0,5', () => {
  const m = muscleSets(T, '2026-09-21', '2026-09-27');
...
test('historia ćwiczenia i rekordy', () => {
  ...
  assert.deepEqual(h.map(x => [x.date, x.bestE1rm, x.topKg]), [['2026-09-21', 60, 45], ['2026-09-28', 63.3, 50]]);
...
test('seria tygodni z treningiem', () => {
  assert.equal(streakWeeks(T, {}, '2026-09-30'), 2);
  assert.equal(streakWeeks(T, {}, '2026-10-20'), 0);
});
