...
// Od 21.09 — kilka dni przed startem planu (25.09.2026, D-086) — do końca suplementów czasowych i dalej
const ALL = [...range('2026-09-21', '2027-03-31')];
const START = '2026-09-25';

test('fazy kalendarzowe D-017 (F0 od 25.09.2026 — D-086; F1 i F2 bez zmian)', () => {
  assert.equal(phaseFor('2026-09-21'), null);
  assert.equal(phaseFor('2026-09-24'), null);
  assert.equal(phaseFor('2026-09-25'), 0);
...
test('każdy dzień 21.09.2026–31.03.2027: 34 sloty z godzinami szablonu (D-004, D-086)', () => {
  const tpl = SRC.dayTemplate.slots.map(s => `${s.from}-${s.to}`).join();
  for (const d of ALL) {
    const r = resolveDay(d);
    assert.equal(r.slots.length, 34, d);
...
test('suplementy: tauryna codziennie (D-014), cynk czw+nd, D-015 25.09.2026–25.03.2027 (D-086)', () => {
  for (const d of ALL) {
    const r = resolveDay(d);
    const s = r.doses.map(x => x.supp);
    assert.ok(s.includes('tauryna'), d);
    assert.equal(s.includes('cynk'), r.weekday === 4 || r.weekday === 7, d);
    const on = d >= START && d <= '2027-03-25';
...
test('CFA: 9 bloków dziennie 25.09–11.11 (D-086), mock w A–E, F wolne (D-019, D-036, D-086)', () => {
  for (const d of ALL) {
    const r = resolveDay(d);
    const cfaSlots = r.slots.filter(s => s.role === 'cfa');
    assert.deepEqual(cfaSlots.map(s => s.from), ['08:00', '09:10', '10:10', '11:20', '12:20', '13:30', '14:30', '15:30', '16:40'], d);
    const n = cfaSlots.reduce((a, s) => a + s.cfa.length, 0);
    if (d >= START && d <= '2026-11-11') assert.equal(n, 9, d); else assert.equal(n, 0, d);
    ...
    if (SRC.cfa.D.mockCFA.includes(d)) {
      ...
      assert.deepEqual(cfaSlots.map(s => s.title), [...'ABCDEFGHI'].map(l => `CFA blok ${l}`), d);
    }
  }
});
...
test('D-039: linia źródła bloku CFA', () => {
  const a = resolveDay('2026-09-25').slots.find(s => s.id === 'slot.0800').cfa[0];
