...
async def run_variant(pw, name, url, mobile):
    ...
    titles = await pg.eval_on_selector_all('.slot .t', 'e => e.map(x => x.textContent)')
    ok(titles.count('Mock CFA — sesja 1') == 2 and titles.count('Wolne') >= 2, f'{tag} dzień mocka: sesje w A–D, E/F „Wolne” (D-036)')
    ok(any('16:23' in t for t in await pg.eval_on_selector_all('.slot .d', 'e => e.map(x => x.textContent)')), f'{tag} obiad opisany jako 16:23 (D-037)')
    if mobile:
